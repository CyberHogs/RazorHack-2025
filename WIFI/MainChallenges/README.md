...
## Description:
* Challenges 1-3 are about wireless network reconnaissance and password cracking.  The players are expected to use the Aircrack-ng suite documentation, which they would have explored in the WiFi training and online searches, to solve these.
...
There is a spy from a rival theme park on the premises! Their last known location is the Dino Fun Park. All we know is that they use a company-provided Samsung phone.
...
<br>**Note:** I made this video before I chose not to include wacker on the competitor's machines to make it harder for them :)
...
* Nmap to find the web server IP
    * `nmap 192.168.1.0/24`
* Clue on the index page, discover the upload script from it
    * `curl 192.168.1.11`
